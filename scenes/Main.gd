...
onready var emote_scene = preload("res://scenes/Emote.tscn");
onready var rng : RandomNumberGenerator = RandomNumberGenerator.new();
onready var emote_callback = JavaScript.create_callback(self, "emote")
onready var party_callback = JavaScript.create_callback(self, "js_set_party")
onready var clear_callback = JavaScript.create_callback(self, "clear_emotes")
...
func _ready():
	...
	var Emote = JavaScript.get_interface("Emote");
...
func _process(delta):
	var activeEmotes = $EmoteContainer.get_child_count()
	var added = 0
...
func _input(event):
	...
	var e = emote_scene.instance();
	var transform : Transform2D = get_viewport_transform();
	var scale : Vector2 = transform.get_scale();
	var newPos = (-transform.origin / scale + get_viewport_rect().size / scale) * Vector2(rng.randf(), rng.randf());
...
func _req_completed(result, response_code, headers, body):
	...
	var emote_image = e.get_node("Image");
